...
from Analysing.Algorithms.MaxPeakIntensityOverPower import MaxPeakIntensityOverPower
...
analysing_algorithms: list[AnalysingAlgorithm] = [
    SameWavelengthOverMaximaAlgorithm(5, 2),
    PreserveMaximaOverPowerAlgorithm(5, 2),
    MaxPeakIntensityOverPower(5, 2)
]

loader = DataLoader("Data")
measurement_groups: list[MeasurementGroup] = loader.load_data()

raw_saver = Saver("Raw", False)
raw_saver.save_measurement_groups_normalized(measurement_groups)

without_bring_to_zero_saver = Saver("OnlySubtractBackground", False)
...
processedSaver = Saver("Results", False)
